...
async function DZSQLiteInsertMusica<T extends { [key: string]: any }>(
  table: SQLiteTable,
  data: T,
  dbname: string = DATABASE_NAME
): Promise<number> {
  ...
  const result = await conn.insert(table).values(sanitizedData).returning();
  const inserted = result[0] as any;
  return inserted?.idTarefa ?? inserted?.id ?? 0;
}
...
async function DZSQLiteUpdateMusica<T extends { [key: string]: any }>(
  id: number,
  newData: Partial<T>,
  table: SQLiteTable,
  idColumn: any, 
  ...
  const conn = DZSQLiteConnect(dbname);
  await conn.update(table).set(newData).where(eq(idColumn, id));
}
...
export { DZSQLiteInsert, DZSQLiteSelect, DZSQLiteUpdate, DZSQLiteDelete, DZSQLiteDeleteAll, DZSQLiteInsertMusica, DZSQLiteUpdateMusica };
